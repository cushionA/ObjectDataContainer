...
{
    /// <summary>
    /// XYZ全軸を使用した3D空間ハッシュコンテナ。
    /// GameObjectをキーとしてデータを格納し、Transform位置を自動追跡する。
    /// Update()で全要素の位置を再ハッシュし、QueryNeighborsで3D近傍検索を行う。
    /// </summary>
    /// <typeparam name="T">格納するデータ型（参照型のみ）</typeparam>
    public class SpatialHashContainer3D<T> : IDisposable where T : class
    {
        /// <summary>
        ...
        /// 3D空間ハッシュコンテナを生成する。
        /// </summary>
        public SpatialHashContainer3D(float cellSize, int maxCapacity)
        {
            _cellSize = cellSize;
        ...
        /// <summary>
        /// 指定した中心座標と半径内にある要素を検索し、距離も返す（3D距離）。
        /// </summary>
        ...
        private int GetCellKey(Vector3 position)
        {
            ...
            int cellY = Mathf.FloorToInt(position.y * _inverseCellSize);
            int cellZ = Mathf.FloorToInt(position.z * _inverseCellSize);
            return cellX * 73856093 ^ cellY * 19349669 ^ cellZ * 83492791;
        }
